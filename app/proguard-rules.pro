...
#  Какой из перечесленных типов методов в Espresso, позволяет проверить состояние объекта и подтвердить, что состояние соответствует критериям?
  #
  # ViewAcception
  # ViewUI
  # ViewCheckable
...
# kotlinx.serialization	- Мультиплатформенная мультиформатная библиотека сериализации, написанная JetBrains
  #Корутины	Функции, которые могут приостанавливать свое выполнение в определенной точке и затем продолжать его с того же места
  #Retrofit	- Библиотека, которая упрощает работу с сетевыми запросами в приложениях на Android
  #MVVM - Архитектурный шаблон, используемый в разработке для разделения пользовательского интерфейса (UI) от бизнес-логики и данных
...
# Имеется программа:
  ...
  #    vаl herоеsList = mutableListOf("John Snow", "David Sprint")
  #     println("Old heroes list: ${heroesList.joinToString()}")
  #     heroesList.аdd(readln()).Ответ Вопрос 27{{sub}}{ println("New heroes list: ${heroesList.joinToString()}") }
  #}
  #
  #Необходимо указать такую пропущенную scope функцию, чтобы программа не скопилировалась.
  ## Ответ: with

 # Какое из определений относится к аббревиатуре DAO?
    #
    # Data Arrival Object
    # Dynamic Access Object
    # Date Access Object
    # Data Access Object +
    # Demo Access Object

  # Какие из аннотаций позволяют выполнить SQL-запрос в методах DAO-интерфейса?
      #
      # @Database
      # @ColumnInfo
      # @Entity
  ...
  # Какие из утверждений справедливы по отношению к интеграционному тестированию?
      #
      # + Проверяет взаимодействие между различными модулями или слоями приложения.
      # Проверяет отдельные модули или функции приложения.
      # + Оценка работы приложения при взаимодействии с базами данных, сетевыми запросами и пользовательским интерфейсом
      # Оценивает функции приложения с точки зрения конечного пользователя
